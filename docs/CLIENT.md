...
ciss-ctl du                                 # per-object sizes + total for your own namespace
ciss-ctl meter                              # receipts + bytes + postage (server-side, per DID)
```

`du` is **self-only** — it reports only your own namespace; cross-DID/store-wide
inspection is an on-box operator task (`ciss usage`), never over the wire. An
operator may lock remote `du` to admins via `CISS_ADMIN_ONLY_DU` (ADR 0003). `du`
needs a v0.5.5+ server (older deployments return `501`).
