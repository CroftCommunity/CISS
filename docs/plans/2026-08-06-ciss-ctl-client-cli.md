...
- **Date:** 2026-08-06 · **Status:** ✅ **SHIPPED v0.5.2 — all phases (0–10) complete + polish/review pass (PR #7); post-ship follow-ons: `ciss-ctl login` for the `did:` app-password credential (PR #8, v0.5.1), `ciss-ctl key list` for pre-key-work orientation (PR #9, v0.5.2), the transfer-plane default flipped to `--via pds` (atproto primary, S3 opt-in; PR #10, v0.5.3), `ls` un-gated for `did:` identities (PR #11, v0.5.4), and **`du` usage inspection** — the first server-side capability: `GET /{did}/du` + `ciss-ctl du`, **self-only over the wire** (cross-DID is never served, not even to admins — store-wide inspection is the on-box `ciss usage`), with an optional `CISS_ADMIN_ONLY_DU` lockdown that restricts remote `du` to admin-pin DIDs (still self-only). ADR 0003 + posture invariant Z9; mutation-checked authz; PR #12 (v0.5.5) then corrected to self-only in v0.5.6. Released with immutable source assets and a pinned Homebrew formula (`brew install croftcommunity/tap/ciss-ctl` → `ciss-ctl 0.5.6`, man page, `brew test` green). **Note:** `du` needs a v0.5.5+ **server** — the deployed VPS returns `501` until redeployed (croft-stack); the full `did:` e2e (login → getServiceAuth → CISS resolves the real `did:plc`) validated against the live VPS with the shipped binary. Testing guide: `docs/CLIENT-TESTING.md` (both-sides, against the VPS)** · **TDD-first**
...
- **Outstanding open items** (redeploy the VPS, toolchain lints, `did:` meter, …):
  see **`docs/TODO.md`**.
